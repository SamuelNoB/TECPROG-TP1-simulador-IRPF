...
from src.exceptions.DescricaoEmBrancoException import (
    DescricaoEmBrancoException,
)
from src.exceptions.ValorRendimentoInvalidoException import (
...
def test_add_invalid_rendimento(descricao, valor):
    controller = RendimentoController()
    with pytest.raises(
        (ValorRendimentoInvalidoException, DescricaoEmBrancoException)
    ):
...
@pytest.mark.parametrize(
    ["rendimentos", "resultado"], rendimento_list_total_value
)
